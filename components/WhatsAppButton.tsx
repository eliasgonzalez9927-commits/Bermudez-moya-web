...
const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({ propertyRef, propertyTitle, variant = 'inline' }) => {
  ...
  if (variant === 'floating') {
    return (
      <a 
        ...
        className="fixed bottom-6 right-6 z-50 bg-brand-red hover:bg-brand-black text-white p-4 rounded-full shadow-2xl transition-transform hover:scale-110 flex items-center justify-center"
  ...
  return (
    <a 
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center justify-center gap-2 bg-brand-red hover:bg-brand-black text-white font-bold py-3 px-6 rounded-lg transition-colors w-full md:w-auto text-lg"
    >
