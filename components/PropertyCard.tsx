...
const PropertyCard: React.FC<PropertyCardProps> = ({ property, onClick, compact = false }) => {
  if (compact) {
    return (
      <div 
        ...
        className="bg-white rounded-2xl p-3 shadow-sm border border-slate-100 flex gap-4 cursor-pointer hover:shadow-md hover:border-blue-200 transition-all group"
      >
        ...
        <div className="flex flex-col justify-center min-w-0">
          ...
          <p className="text-blue-600 font-black text-sm mb-1">{property.currency} {property.price.toLocaleString()}</p>
  ...
  return (
    <div 
      onClick={() => onClick(property.id)}
      className="bg-white rounded-[2.5rem] shadow-[0_15px_50px_rgba(0,0,0,0.03)] overflow-hidden hover:shadow-[0_20px_60px_rgba(0,0,0,0.08)] transition-all cursor-pointer border border-slate-50 group flex flex-col h-full"
    >
      {/* Image Container */}
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-6 left-6 flex flex-col gap-2">
          <span className="bg-blue-600 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em] shadow-xl">
            {property.operation}
          </span>
          <span className="bg-slate-900/80 backdrop-blur-md text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em] shadow-xl">
            {property.type}
          </span>
        </div>
        ...
        {/* Image count badge */}
        {property.images.length > 1 && (
          <div className="absolute top-6 right-6 bg-white/20 backdrop-blur-md text-white text-[10px] px-3 py-1.5 rounded-full flex items-center gap-1.5 font-black border border-white/30 shadow-lg">
        ...
        {property.features.rooms > 0 && (
          <div className="absolute bottom-6 left-6 right-6 flex gap-2">
            <div className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-2xl flex items-center gap-2 shadow-lg">
              <span className="text-xs font-black text-slate-900">{property.features.rooms} Amb.</span>
            </div>
            <div className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-2xl flex items-center gap-2 shadow-lg">
              <span className="text-xs font-black text-slate-900">{property.features.m2Total} m²</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-8 flex flex-col flex-1">
        <div className="flex justify-between items-start mb-4 gap-2">
          <div className="min-w-0 flex-1">
            <h3 className="text-xl font-black text-slate-900 truncate leading-tight group-hover:text-blue-600 transition-colors mb-1">{property.title}</h3>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest truncate">{property.neighborhood}, {property.location.city}</p>
          </div>
        ...
        <div className="mt-auto flex items-center justify-between pt-6 border-t border-slate-50">
          <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">REF: {property.ref}</span>
